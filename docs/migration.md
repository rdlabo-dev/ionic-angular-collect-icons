...
First update the package, then preview the migration:
...
Run the collector normally to apply it:

```bash
npx @rdlabo/ionic-angular-collect-icons
```

The normal collector command performs this migration automatically; the
`--initialize` option is not required for an initializer generated by an older
release. The resulting code is:
...
During a normal collector run, the automatic migration only rewrites the
ternary initializer generated by older releases:
...
When `--initialize true` is explicitly requested, the conventional
